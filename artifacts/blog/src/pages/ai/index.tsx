...
import { VISIBLE_AI_MODES_MAIN, VISIBLE_AI_SPECIAL_MODES, type AiModeConfig } from "@/components/platform/ai-config";
import { ArrowRight, MessageSquare, Sparkles } from "lucide-react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { HubSeoIntro } from "@/components/hub/hub-seo-intro";

function ModeCard({ mode, highlight = false }: { mode: AiModeConfig; highlight?: boolean }) {
  const Icon = mode.icon;
  return (
    <Link
      href={mode.href}
      className={cn(
        "group relative flex h-full flex-col rounded-2xl border bg-card p-5 sm:p-6 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5",
        highlight ? "border-primary/25 ring-1 ring-primary/10" : "border-border/60",
      )}
    >
      <div
        className={cn(
          "mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl ring-1 ring-border/40",
          mode.iconBg,
        )}
      >
        <Icon className={cn("h-5 w-5", mode.accent)} />
      </div>
      <h3 className="text-base font-bold group-hover:text-primary transition-colors">{mode.label}</h3>
      <p className="mt-2 flex-1 text-sm text-muted-foreground leading-relaxed line-clamp-3">
        {mode.description}
      </p>
      <p className="mt-3 text-xs text-muted-foreground/80 italic line-clamp-2">“{mode.prompts[0]}”</p>
  ...
  return (
    <AiLandingShell>
      <SeoHead
        ...
        description="Explain, debug, and convert code with a developer-tuned AI assistant."
      />
        ...
        <div className="container mx-auto max-w-6xl px-4 py-16 md:py-24">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mx-auto mt-5 max-w-xl text-base md:text-lg text-muted-foreground leading-relaxed">
              Six specialized modes — from debugging stack traces to explaining errors. Paste a snippet, pick a
          ...
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {VISIBLE_AI_MODES_MAIN.map((mode) => (
                <ModeCard key={mode.id} mode={mode} highlight={mode.id === "chat"} />
              ))}
            </div>
          </div>

          {VISIBLE_AI_SPECIAL_MODES.length > 0 && (
            <div>
              <h3 className="mb-4 text-center text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Specialized
              </h3>
              <div className="mx-auto grid max-w-3xl gap-4 sm:grid-cols-2">
                {VISIBLE_AI_SPECIAL_MODES.map((mode) => (

...
function buildSections(paragraphs, path) {
  const titles = SECTION_TITLES[path] ?? DEFAULT_SECTION_TITLES;
  const sections = [];
  for (let i = 0; i < paragraphs.length; i += 2) {
    const slice = paragraphs.slice(i, i + 2);
...
/** @typedef {{ path: string, title: string, description: string, h1: string, h2: string, paragraphs: string[], sections: { title: string, paragraphs: string[] }[], sharedSection: { title: string, paragraphs: string[] } }} SeoPage */
...
export const HUB_SEO_PAGES = hubSeoContent.pages.map((page) => {
  ...
  return {
    ...page,
    paragraphs: [...pageParagraphs, ...hubSeoContent.sharedParagraphs],
    sections: buildSections(pageParagraphs, page.path),
    sharedSection: {
      title: "Explore TechVentry",

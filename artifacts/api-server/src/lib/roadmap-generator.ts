import type { RoadmapPayload, RoadmapStep } from "@workspace/db";
...
const GOAL_LABELS: Record<RoadmapGoal, string> = {
...
type StepTemplate = Omit<RoadmapStep, "estimatedWeeks"> & { weeksByLevel: Record<string, number> };

const GOAL_STEPS: Record<RoadmapGoal, StepTemplate[]> = {
  "frontend-developer": [
...
export function generateRoadmapPayload(
  goal: RoadmapGoal,
  currentLevel: (typeof ROADMAP_LEVELS)[number],
): RoadmapPayload {
  const templates = GOAL_STEPS[goal] ?? GOAL_STEPS["full-stack-developer"];
  const levelKey = currentLevel;

  const steps: RoadmapStep[] = templates.map((t) => ({
    key: t.key,
    title: t.title,
    description: t.description,
    learnHref: t.learnHref,
    interviewTopic: t.interviewTopic,
    estimatedWeeks: t.weeksByLevel[levelKey] ?? t.weeksByLevel.beginner,
  }));
  ...
  return {
    ...
    currentLevel,
    title: `${label} Roadmap`,
    summary: `Personalized ${levelKey} path to become a ${label}. Estimated ${totalWeeks} weeks at a steady pace.`,
    totalWeeks,
    steps,
    resources: [
